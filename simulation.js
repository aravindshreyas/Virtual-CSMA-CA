...
function checkIFS() {
  for (let n = 0; n < 6; n++) {
    if (n != y - 1) clscts[n].innerHTML = "CTS : Received";
  }
  ...
  document.getElementById("channel").innerHTML = "Channel : Occupied";
  swap();
...
function sendFrame() {
  ...
  if (n > 0.1) {
    ...
    if (k[x - 1] >= 15) {
      ...
      clsdata[x - 1].innerHTML = clsdata[y - 1].innerHTML =
        "Data Transfer : - ";
      alert("Acknowledgement not received,trying again !");
      channel = 0;
      for (let i = 0; i < 6; i++) {
...
function setDefault() {
  ...
  var n = document.querySelectorAll("#inputGroupSelect02");
  for (let j = 0; j < 6; j++) {
    n[j].value = "Choose Destination";
  }
}
